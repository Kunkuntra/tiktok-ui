...
import { CloseTabs } from '~/components/Control';
...
function UpdateForm({ handleEdit }) {
  const { user, token, setOpenFormEdit } = useAuth();

  const data_user = {
    myWebsite: user.website_url,
  ...
  const handleUpdateUser = () => {
    const fetchApi = async () => {
      await update(token, valueFirstName, valueLastName, valueWebsite, valueBiO);
  ...
  return (
    <div className={cx('form-wrapper')}>
      <section className={cx('modal-update')}>
        <span onClick={handleEdit} className={cx('close-btn')}>
          x
        </span>
