...
import { ArrowDownIcon, CheckBoxIcon } from '~/components/Icons';
...
function FormMainUpload({ src = '', nameFile = '' }) {
  ...
  const { user, token, setDataForm, setOpenFormDiscard } = useAuth();
  ...
  const handleUploadVideo = () => {
    const fetchApi = async () => {
      ...
      if (data?.errCode) {
        alert(`Upload failed: ${data.message || 'Unexpected error'}`);
      } else {
